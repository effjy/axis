...
static void add_feature_row(GtkWidget *box, const char *title, const char *desc, const char *icon_symbol) {
    ...
    gtk_label_set_markup(GTK_LABEL(title_label), g_strdup_printf("<span weight='bold' size='large' color='#A855F7'>%s</span>", title));
...
static void on_about_clicked(GtkWidget *widget, gpointer data) {
    ...
    gtk_label_set_markup(GTK_LABEL(name_label), "<span size='large' weight='bold' color='#F1F1F0'>Axis - Encrypted Disk Manager</span>");
    gtk_box_pack_start(GTK_BOX(main_vbox), name_label, FALSE, FALSE, 0);
    
    GtkWidget *ver_label = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(ver_label), g_strdup_printf("<span color='#A855F7'>Version %s</span>", APP_VERSION));
...
static GdkPixbuf* load_app_logo(int size) {
    ...
    char *path_logo1 = g_build_filename(cwd, "axis_logo.png", NULL);
    char *path_logo2 = g_build_filename(cwd, "axis_logo.png", NULL);
    char *path1 = g_build_filename(cwd, "axis_logo.png", NULL);
    char *path2 = g_build_filename(cwd, "axis_logo.png", NULL);
    ...
    const char *paths[] = {
        ...
        "axis_logo.png",
        "axis_logo.png",
        "/usr/share/axis/axis_logo.png",
        "/usr/local/share/axis/axis_logo.png",
        "/usr/share/icons/hicolor/64x64/apps/axis_logo.png",
        path1,
        path2,
        "axis_logo.png",
        "axis_logo.png",
...
static void on_activate(GtkApplication *app, gpointer user_data) {
    ...
    /* Dynamically resolve axis_logo.png path */
    ...
    if (splash_path) {
        ...
        splash_pb = gdk_pixbuf_new_from_file_at_scale("axis_logo.png", 300, 300, TRUE, NULL);
    }
...
#define SECRET_KEY "Axis-Super-Secret-Key-2026"
        ...
        if (filename) {
            ...
            if (verify_license_file(state->path_buffer)) {
                ...
                GtkWidget *err_dialog = gtk_message_dialog_new(
                    ...
                    GTK_MESSAGE_ERROR,
                    GTK_BUTTONS_OK,
                    "Invalid license file! Please select a valid 1-year Axis license."
                );
...
static int __attribute__((unused)) check_license_verification(void) {
    ...
    snprintf(license_save_path, sizeof(license_save_path), "%s/.axis_license", home);
    snprintf(state_path, sizeof(state_path), "%s/.axis_state", home);
    ...
    gtk_window_set_title(GTK_WINDOW(dialog), "License Activation - Axis");
    ...
    gtk_css_provider_load_from_data(cp,
        ...
        ".action-btn { background: #00E5FF; color: #060608; font-weight: bold; border: none; border-radius: 6px; padding: 8px 16px; }\n"
        ".action-btn:hover { background: #00B8D4; color: #FFFFFF; }\n"
        ".trial-btn { background-color: #121214; color: #F1F1F0; border: 1px solid #252529; border-radius: 6px; padding: 8px 16px; font-weight: bold; }\n"
        ".trial-btn:hover { background-color: #252529; border-color: #00E5FF; color: #F1F1F0; }\n"
    ...
    GtkWidget *lbl_title = gtk_label_new("Axis Activation");
    ...
    if (tries_left > 0) {
        ...
        gtk_label_set_text(GTK_LABEL(lbl_desc),
            "Axis Manager requires a valid 1-year license to run.\n"
        ...
        gtk_label_set_text(GTK_LABEL(lbl_desc),
            "The trial period has expired. You can no longer run Axis.\n"
    ...
    if (dstate.code == 2) {
        ...
        GtkWidget *success_dialog = gtk_message_dialog_new(
            ...
            GTK_MESSAGE_INFO,
            GTK_BUTTONS_OK,
            "License activated successfully! Thank you for choosing Axis."
        );
