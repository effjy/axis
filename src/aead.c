...
static void *seg_worker(void *arg) {
    ...
    uint8_t iv[12];
    memcpy(iv, w->nonce, 12);
    uint8_t idx[8];
    store_le64(idx, w->seg_idx);
    for (int i = 0; i < 8; i++) iv[4 + i] ^= idx[i];

    if (w->is_decrypt) {
        EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
        EVP_DecryptInit_ex(ctx, EVP_aes_256_ctr(), NULL, w->key, iv);
        int outl;
        EVP_DecryptUpdate(ctx, w->out, &outl, w->in, w->len);
        EVP_DecryptFinal_ex(ctx, w->out + outl, &outl);
        EVP_CIPHER_CTX_free(ctx);

        if (w->use_parallel_mac) {
            uint8_t dummy_cipher[w->len];
            aes256gcm_encrypt(w->out, w->len, w->key, iv, 12, idx, 8, dummy_cipher, w->mac);
        }
    } else {
        aes256gcm_encrypt(w->in, w->len, w->key, iv, 12, idx, 8, w->out, w->mac);
    }
...
int permut2048_aead_encrypt_stream(FILE *fout, const char *in_path,
    ...
    /* V3: Encrypted Magic */
    uint8_t iv[12] = {0};
    uint8_t idx[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    memcpy(iv, nonce, 12);
    for (int i = 0; i < 8; i++) iv[4 + i] ^= idx[i];
    ...
    EVP_EncryptInit_ex(ectx, EVP_aes_256_ctr(), NULL, key, iv);
    int outl;
    EVP_EncryptUpdate(ectx, enc_magic, &outl, v3_magic, 8);
    EVP_EncryptFinal_ex(ectx, enc_magic + outl, &outl);
    ...
    if (memcmp(magic_or_nonce, "KRAKKEN1", 8) == 0) {
        ...
        if (fread(nonce_remainder, 1, 4, fin) == 4) {
            ...
            if (fread(enc_magic, 1, 8, fin) == 8) {
                ...
                EVP_DecryptInit_ex(dctx, EVP_aes_256_ctr(), NULL, key, iv);
                int outl;
                EVP_DecryptUpdate(dctx, dec_magic, &outl, enc_magic, 8);
                EVP_DecryptFinal_ex(dctx, dec_magic + outl, &outl);
